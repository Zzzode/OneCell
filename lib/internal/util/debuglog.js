...
// `debugImpls` and `testEnabled` are initialized to safe defaults so that
// debuglog() works even when `initializeDebugEnv()` hasn't been called yet
// (e.g. in edge runtime environments that skip the full Node.js bootstrap).
let debugImpls = { __proto__: null };
